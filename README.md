...
- **Q. 정답을 왜 텍스트가 아니라 번호(1~4)로 저장했나요?** — A. 선택지 문구를 나중에 고쳐도 정답이 흔들리지 않고,
  `Quiz.is_correct`가 `int(choice) == self.answer` 단순 비교로 끝납니다. 문자열 비교는 공백·대소문자·중복 텍스트에 취약합니다.
- **Q. `state.json`이 깨지면 어떻게 되나요?** — A. `QuizGame.load()`가 `JSONDecodeError·OSError·ValueError·KeyError·TypeError`를
  잡아 안내 후 기본 퀴즈로 복구합니다. 깨진 JSON 문법, 일부 필드 타입 오류(예: `quizzes`가 숫자), 그리고 **최상위가 객체가 아닌 파일(`[]`·`null`)도**
  `isinstance(data, dict)` 검사로 손상 처리해 복구하므로 프로그램은 계속됩니다. 종료 신호(Ctrl+C/EOF)도 `main.py`에서 저장 후 안전 종료합니다.
- **Q. 힌트를 봤는데 정답이면 왜 점수가 안 오르나요?** — A. 의도된 정책입니다. `used_hint=True`면 정답이어도 `correct`를 올리지
  않습니다(`is_correct(choice) and not used_hint`). 힌트에 "그 문제 점수 포기"라는 비용을 부여한 것입니다.
- **Q. 5문제 풀 때와 3문제 풀 때 최고 점수 비교가 공정한가요?** — A. 점수는 `int(correct/count*100)` 백분율이라 0~100 한 척도로
  맞춰져 원점수보다 비교하기 쉽습니다. 다만 문제 수가 적으면 눈금이 굵어(3문제 → 0·33·67·100) 같은 100점이라도 정보량은 다릅니다.
- **Q. `Quiz`와 `QuizGame`을 왜 나눴나요?** — A. 한 문제의 데이터·유효성 검사·직렬화(`to_dict/from_dict`)는 `Quiz`가,
  퀴즈 목록·최고점수·기록·파일 IO·메뉴 흐름은 `QuizGame`이 맡습니다. 책임이 섞이지 않아 수정·설명이 쉽습니다.
- **Q. 랜덤 출제에서 같은 문제가 두 번 나오지 않나요?** — A. `random.sample`은 비복원 추출이라 같은 목록 항목을 두 번 뽑지 않습니다.
  다만 사용자가 **똑같은 내용의 문제를 중복 등록**했다면 그 둘이 함께 뽑힐 수는 있습니다(등록 시 중복 방지는 하지 않음).
  또 힌트가 없는 문제에 `h`를 누르면 `has_hint()`가 False라 "힌트 없음"을 안내하고 재입력을 받습니다(점수 영향 없음).
